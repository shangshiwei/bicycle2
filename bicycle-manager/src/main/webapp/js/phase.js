...
var vm = new Vue({
		...
		methods:{
			goPage: function(pageNum){
				...
				this.$http.get("../main/msphase/select",params).then(
						(resp) => {
			...
			// 点击修改权限按钮
			updateDiv: function(phaseId){
				this.isUpdate = true;
				this.msPhase = {};//清空表单数据
				this.valid = [true,true];
				this.flag = true;
				this.msPhase.phaseId = phaseId;
			},
			// 点击删除权限按钮
			deleteDiv: function(phaseId){
				this.msPhase = {};//清空表单数据
				this.msPhase.phaseId = phaseId;//保存该行id
				var params = {
						"phaseId" : phaseId,
					}
				url = "msphase/delete";
